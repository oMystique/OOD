...
int main()
{
	...
	{
		auto beverage =
			make_unique<CCapuccino>(2)					// Наливаем двойную порцию капучино
			<< MakeCondiment<CCinnamon>()	// сыпанем корицы
			<< MakeCondiment<CCoconutFlakes>(8)		// посыпаем кокосовой стружкой
			<< MakeCondiment<CChocolateCrumbs>(1200); // добавим немного шоколадной крошки, для вкуса

														// Выписываем счет покупателю
		cout << beverage->GetDescription() << " costs " << beverage->GetCost() << endl;
	}
	//После того, как мы попробовали замечательное двойное капуччино с щепоткой сахара, что-то нам захотелось чая.
	//И как же нам повезло, когда обнаружилось, что в чайной карте появилось разнообразие.
	{
		auto beverage =
			make_unique<CTea>(TeaType::chefir)	// Просим налить нам чая
			<< MakeCondiment<CIceCubes>(2, IceCubeType::Dry)	// После килограмма шоколада нам захотелось чего-то прохладненького
			<< MakeCondiment<CSyrup>(SyrupType::Maple);		// Настоящий британский изыск! 

													  // Выписываем счет покупателю
		cout << beverage->GetDescription() << " costs " << beverage->GetCost() << endl;
	}
	//А теперь неплохо было бы отведать большого молочного коктеля
	{
		auto beverage =
			make_unique<CMilkshake>(MilkshakePortionType::big)
